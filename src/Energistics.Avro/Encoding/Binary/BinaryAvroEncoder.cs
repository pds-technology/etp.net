...
{
    /// <summary>
    /// A class for binary Avro encoding
    /// </summary>
    public class BinaryAvroEncoder : BinaryPrimitiveEncoder, IAvroEncoder
    {
        public BinaryAvroEncoder(Stream stream)
            : base(stream)
        {
        }

        private void EncodeBlockStart(int count, long? size)
        {
            if (size == null)
                EncodeInt(count);
            else
            {
                EncodeInt(-count);
                EncodeLong(size.Value);
            }
        }

        public void EncodeFixed(long byteCount, byte[] value) => EncodeRaw(value);
        ...
        public void EncodeArrayStart(int totalCount, int blockCount, long? blockByteCount = null) => EncodeBlockStart(totalCount, blockByteCount);
        ...
        public void EncodeMapStart(int totalCount, int blockCount, long? blockByteCount = null) => EncodeBlockStart(totalCount, blockByteCount);
